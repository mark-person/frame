...
public class AuthFilterUtils {
    ...
    public static LoginAccount getLoginAccout(HttpServletRequest request, HttpServletResponse response, String uri) {
       ...
       // request.setAttribute("insertOrUpdate", true);
        ...
        // 所有用户进入/auth/index/(菜单、修改密码等)
        if (uri.startsWith("/auth/index/")) {
            return account;
        }

        // 主帐号进入/child/菜单不拦截
        if (account.isMainAccount() && uri.startsWith("/auth/authChild/")) {
            return account;
        }
    ...
    private static LoginAccount filterUri(HttpServletRequest request, String uri, LoginAccount account) {
        ...
        if (paraEnum.hasMoreElements()) {
        	...
        	testUriList.add(uri + "?" + name + "=" + value);
        }
        ...
        for (String testUri : testUriList) {
            // 取得URI对应的index
            Integer testIndex = filterService.getIndexFromUri(testUri);
            if (testIndex == null) {
                continue;
            }
            ...
            if (!account.isMainAccount()) {
                ...
                if (!userGrantBitset.get(testIndex)) {
                    continue;
                }
            }

            if (grantBitset.get(testIndex)) {
                String realUri = uri;
                if (testUri.contains("?")) {
                    realUri = testUri;
                }
                // uri合法访问, 通过request.setAttribute设置操作权限 传入真实URI
                setOperationPermission(request, filterService.getOpUri(realUri), grantBitset);
                return account;
            }
            missUri = false;
        }
        ...
        if (missUri) {
            ...
            throw new SecuritException(ExceptionCode.SECURITY_URL, "Unauthorized.miss uri:" + uri);
        } else {
            // 帐号没有该URI的权限
            throw new SecuritException(ExceptionCode.SECURITY_URL, "Unauthorized.forbiddens:" + uri);
        }
    }
    ...
    private static void setOperationRequest(HttpServletRequest request, String uri) {
        ...
        // /test/saveTest改名成_test_saveTest, TODO 改成最一后一个
        request.setAttribute(uriItem[uriItem.length - 1], true);
    }
}
