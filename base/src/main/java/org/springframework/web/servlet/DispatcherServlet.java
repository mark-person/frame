	...
	private void applyDefaultViewName(HttpServletRequest request, @Nullable ModelAndView mv, Object mappedHandler) throws Exception {
		...
		if (mv != null && !mv.hasView()) {
			...
			if ("HandlerMethod".equals(mappedHandler.getClass().getSimpleName())) {
				...
				String[] item = defaultViewName.split("/");
				String method = item[item.length - 1];
				
				if (p.length == 5) {
					defaultViewName = serviceName + "/" + moduleName + "/" + method;
				}
				if (p.length == 6) {
					defaultViewName = serviceName + "/" + moduleName + "/" + p[5] + "/" + method;
				}
				if (p.length == 7) {
					defaultViewName = serviceName + "/" + moduleName + "/" + p[5] + "/" + p[6] + "/" + method;
				}
